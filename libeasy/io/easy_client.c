...
int easy_client_dispatch(easy_io_t *eio, easy_addr_t addr, easy_session_t *s)
{
    ...
    if (eio->stoped || (ioth != NULL && ioth->iot)) {
        easy_error_log("easy_io_dispatch is failure: stoped: %d, ioth: %p, s=%p\n",
                       eio->stoped, ioth, s);
        return EASY_ERROR;
    }

    index = easy_hash_code(&addr, sizeof(easy_addr_t), 7);
...
void *easy_client_list_find(easy_hash_t *table, easy_addr_t *addr)
{
    ...
    // foreach
    while(list) {
        if (memcmp((void *)(long)list->key, addr, sizeof(easy_addr_t)) == 0) {
